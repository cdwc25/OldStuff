void blinky (int speed) {
  if (scaredmode == false) image (blinky, bx, by, 30, 30);
  if (scaredmode == true) image (scared, bx, by, 30, 30);
  ...
  //Movement
  if (btargetx > bx) bx = bx + speed;
  if (btargetx < bx) bx = bx - speed;
  if (btargety > by) by = by + speed;
  if (btargety < by) by = by - speed;
}
