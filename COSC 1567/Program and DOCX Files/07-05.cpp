...
class BankAccount
{
 public:
    ...
    double getBalance( ) const { return (accountDollars + accountCents*0.01);}//inline function
 ...
 private:
    ...
    int dollarsPart(double amount) const { return static_cast<int>(amount); }//inline f.
    
    int centsPart(double amount) const;
 
    int round(double number) const   //inline function
    { return static_cast<int>(floor(number + 0.5)); }
 
    double fraction(double percent) const { return (percent/100.0); } //inline f.
};
